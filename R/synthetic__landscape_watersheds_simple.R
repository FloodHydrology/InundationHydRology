...
#Export Raster
writeRaster(dem,"dem_ws_bathymetry_simple.asc",overwrite=TRUE)
...
####################################################################################
#Step 6: Create Wetlands Raster for Brian
####################################################################################
#Remove data from dem
dem<-dem*0

#Create function to define water surface elevation in dem
fun<-function(WetID){
  #Setup~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #define variables
  area_max<-pnts$area_m2[pnts$WetID==WetID]
  x<-pnts$x[pnts$WetID==WetID]
  y<-pnts$y[pnts$WetID==WetID]
  z<-pnts$z[pnts$WetID==WetID]-pnts$ws_depth[pnts$WetID==WetID]
  ...
  #Represent water surface with points~~~~~~~~~~~~~~~~~~~~~~~~
  #Calculate variables
  radius<-(area_max/pi)^0.5
  n<-round(radius/cell_size)
  ...
  for(i in 1:(n+1)){
    area_ring<-pi*(i*cell_size)^2
    df<-data.frame(circle.fun(area_ring, x,y),z)
    colnames(df)<-c("x","y","z")
    bath<-rbind(bath,df)
  }
...
#Export Raster
writeRaster(dem,"water_surface_elevation.asc",overwrite=TRUE)
